...
import type { AspectRatio, OutputType } from '../types';
...
export const PromptInput: React.FC<PromptInputProps> = ({
  ...
  const OutputTypeButton: React.FC<{ value: OutputType; label: string }> = ({ value, label }) => {
    const isActive = outputType === value;
    return (
      <button
  ...
  return (
    <div className="bg-slate-800/50 rounded-lg p-6 shadow-2xl border border-slate-700/50 backdrop-blur-sm">
      <label htmlFor="prompt-input" className="block text-lg font-medium text-slate-300 mb-2">
        Enter your prompt idea(s)
      </label>
      <textarea
        id="prompt-input"
        value={userInput}
        onChange={(e) => setUserInput(e.target.value)}
        placeholder="Enter one prompt per line for batch processing.
e.g., A cyborg skeleton glowing with golden electricity..."
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        <div>
          <span className="block text-sm font-medium text-slate-300 mb-2">Output Type:</span>
          <div className="flex gap-2 w-full">
            <OutputTypeButton value="image" label="Image" />
            <OutputTypeButton value="video" label="Video" />
            <OutputTypeButton value="both" label="Both" />
          </div>
        </div>
        <div>
      ...
      <div className="mt-6">
        <button
          ...
          className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold py-3 px-8 rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100 flex items-center justify-center gap-2"
        >
          {isLoading ? (
            <>
